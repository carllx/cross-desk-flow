...
class TestProductShellAppIPC(unittest.TestCase):
    ...
    def test_voice_ducking_ui_state_and_slider(self):
        ...
        self.assertEqual(app.lbl_voice_title.cget("text"), "Local Mac Audio")
        self.assertEqual(app.lbl_voice_status.cget("text"), "Active")
        self.assertEqual(app.lbl_duck_title.cget("text"), "When Mac audio is active: 35%")
        self.assertEqual(int(round(app.duck_scale.get())), 35)

        # Move slider
        ipc_calls.clear()
        app.on_duck_slider_change("50")
        self.assertEqual(app.lbl_duck_title.cget("text"), "When Mac audio is active: 50%")
